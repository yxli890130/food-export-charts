...
- 输入草稿与已应用关键词分离：输入时列表和 URL 保持不变，点击“搜索”或按 Enter 后才更新结果
- 输入已修改但未提交时显示明确操作提示，搜索按钮在无可提交内容时禁用
- 结果数量反馈 + 无结果可行动提示
...
- 浏览器验证：输入 `0712` 时结果和 URL 不变；点击“搜索”或按 Enter 后定位到 HS0712、高亮标记可见且 URL 写入 `q=0712`
- 搜索与 HS2 范围、机会初筛可叠加
